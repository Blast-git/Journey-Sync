  ...
  const fetchEarningsData = async () => {
    try {
      ...
      const todayEarnings = bookings
        ?.filter(b => b.rides.departure_date === todayStr)
        .reduce((sum, b) => sum + Number(b.total_price), 0) || 0;

      const thisWeekEarnings = bookings
        ?.filter(b => b.rides.departure_date >= weekAgoStr)
        .reduce((sum, b) => sum + Number(b.total_price), 0) || 0;

      const thisMonthEarnings = bookings
        ?.filter(b => b.rides.departure_date >= monthAgoStr)
        .reduce((sum, b) => sum + Number(b.total_price), 0) || 0;

      const allTimeEarnings = bookings
        ?.reduce((sum, b) => sum + Number(b.total_price), 0) || 0;

      // Calculate last week earnings for growth comparison
      const lastWeekEarnings = bookings
        ?.filter(b => b.rides.departure_date >= twoWeeksAgoStr && b.rides.departure_date < weekAgoStr)
        .reduce((sum, b) => sum + Number(b.total_price), 0) || 0;
      ...
      // Generate daily earnings for the last 7 days
      const last7Days = Array.from({ length: 7 }, (_, i) => {
        const date = new Date(today.getTime() - i * 24 * 60 * 60 * 1000);
        const dateStr = date.toISOString().split('T')[0];
        
        const dayBookings = bookings?.filter(b => b.rides.departure_date === dateStr) || [];
        const dayEarnings = dayBookings.reduce((sum, b) => sum + Number(b.total_price), 0);
