...
const Index = () => {
  ...
  // UPDATED: Better auto-redirect logic with role checking
  useEffect(() => {
    if (!loading && user && profile) {
      // Only auto-redirect if user has a clear role preference
      // Otherwise let them choose in AppSelection
      if (profile.role === 'driver') {
        navigate('/driver', { replace: true });
      } else if (profile.role === 'passenger') {
        navigate('/passenger', { replace: true });
      } else {
        // If no specific role or role is unclear, go to app selection
        navigate('/app-selection', { replace: true });
      }
    }
